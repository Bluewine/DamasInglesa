...
//        }
    ...
    private void jugar(int jugador) {
        ...
        if (sePudoComer || sePudoMover) {
            ...
            boolean reinaExistente = this.reinasA.contains(new int[]{oldRow,oldCol});
            ...
            if (reinaExistente || row == 0) {
                ...
                if (reinaExistente)
                    ...
                    this.reinasA.remove(new int[]{oldRow,oldCol});                            
            }
    ...
    private void jugarRandom(int jugador) {
        ...
        for (int i = 0; i < tablero.length; i++) {
            for (int j = 0; j < tablero[0].length; j++) {
                ...
                if (jugador == this.jugadorAgente) { // si es el agente

                    k = i-2; l=j-2; m=j+2;

                    // intentar comer, noroeste.                        
                    if (isInRange(k,l) && tablero[k][l] == 0 && tablero[k+1][l+1] == contrario) {
                        filas.add(k); columnas.add(l);  // guardar la fila y columna hacia el cual podria moverse.
                    ...
                    if (isInRange(k,m) && tablero[k][m] == 0 && tablero[k+1][m-1] == contrario) {
                        filas.add(k); columnas.add(m);
                    ...
                    if (!huboMov) {
                        ...
                        // moverse a noroeste
                        if (isInRange(k, j) && tablero[k][l] == 0) {
                            filas.add(k); columnas.add(l);
                            origRowCol.add(new int[]{i,j});
                        ...
                        // la ficha a moverse es una reina
                        if (!huboMov && this.reinasA.contains(new int[]{i,j})) {
                    ...
                    k = i-2;
                    // la ficha a moverse es una reina
                    if (this.reinasB.contains(new int[]{i,j})) {
                    ...
                    if (!huboMov) {
                        ...
                        // la ficha a moverse es una reina
                        if (!huboMov && this.reinasB.contains(new int[]{i,j})) {
                            k = i-1;
        ...
        if (huboMov) {
            ...
            swap(origRowCol.get(random)[0], origRowCol.get(random)[1],
                    filas.get(random), columnas.get(random));
            
            // si en la movida realizo alguna captura de la ficha del contrario
            if (huboCaptura.get(random)) {
                ...
                if (jugador==this.jugadorAgente) {
                    ...
                    if (filas.get(random) == 0 || isReina) {
                        // registrar en la lista de reinas, la nueva posicion
                        this.reinasA.add(new int[]{filas.get(random), columnas.get(random)});
                        // si era una reina existente, remueve la vieja posicion.
                        if (isReina) this.reinasA.remove(origRowCol.get(random));
                    }
                    // actualizar ultimo tablero    
                    ...
                    // si la ficha movida se convierte en nueva reina, o es una ya existente
                    if (filas.get(random) == 7 || isReina) {
                        // registrar en la lista de reinas, la nueva posicion
                        this.reinasB.add(new int[]{filas.get(random), columnas.get(random)});
                        // si era una reina existente, remueve la vieja posicion.
                        if (isReina) this.reinasB.remove(origRowCol.get(random));
                    }
                }
            }
        }
...
//        }
    ...
    private void jugarHumano(int jugador) { // siempre es oponente
        ...
        int contrario = jugador; // solamente el oponente del agente invoca a este metodo.
        
        // evitar desborde y verificar que la ficha elegida para mover sea la correcta (del enemigo)
        if (isInRange(X1, Y1) && isInRange(X2, Y2) && tablero[X1][Y1] == contrario) {
            
            boolean isReina = this.reinasB.contains(new int[]{X1,Y1});
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int q = 0; q < qRates.length; q++) {
            ...
            for (int k = 0; k < totalExperiments; k++) {
                ...
                RLAg_Damas ag = new RLAg_Damas(trainingCount);
