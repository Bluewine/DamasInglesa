...
public class RLAgent {
	...
	private int jugadorAgente = 1;
        ...
        private int size = 8;
        private int[] reinaA, reinaB;
        private int fichasA, fichasB;
        private int fwrdA, fwrdB;
        boolean noMovA = false, noMovB = false;
	...
	public void reset(boolean entrenar) {
		...
		this.gameResult = 0;
                ...
                this.noMovA = false;
                this.noMovB = false;
                this.reinaA[0] = -1; this.reinaB[0] = this.reinaA[0];
                this.reinaA[1] = -1; this.reinaB[1] = this.reinaA[1];
                this.fwrdA = -1;
                this.fwrdB = 1;
	}
	...
	private void updateProbability(int[][] tablero, double nextStateProb, int jugador) {
        ...
        private boolean isKing(int i, int j, int jugador) {
            return jugador == this.jugadorAgente ?
                    (this.reinaA[0] == i && this.reinaA[1] == j) :
                    (this.reinaB[0] == i && this.reinaB[1] == j) ;
        }
		...
		double maxProb = Integer.MIN_VALUE;
                for (int i=0; i<this.size; i++)
                    for (int j=0; j<this.size; j++)
                        if (tablero[i][j] == jugador) { // por cada ficha del jugador
                            if (i+this.fwrdA*this.size)
                        }
